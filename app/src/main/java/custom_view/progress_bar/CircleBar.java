package custom_view.progress_bar;
...
/**
 * http://blog.csdn.net/u012721933/article/details/51787449
 */
public class CircleBar extends ProgressBar {
    ...
    private int mTextColor = getResources().getColor(R.color.white);//默认文字颜色
    private int mWheelColor = getResources().getColor(R.color.progressColor);//默认圆环颜色

    private String mText;
    private String mTextDes;//文字的描述
    private int mTextDesSize;//描述文字的大小
    private float mCount;//为了做动画
    ...
    private void init(AttributeSet attrs, int defStyle) {
        //初始化一些值
        circleStrokeWidth = dip2px(getContext(), 10);//圆环粗细
        pressExtraStrokeWidth = dip2px(getContext(), 2);
        mTextSize = dip2px(getContext(), 30);//数字显示大小
        mTextDesSize = dip2px(getContext(), 13);//描述文本大小
        mDistance = dip2px(getContext(), 18);//文字距离
        //外圆环的画笔
        ...
        mDefaultWheelPaint.setColor(getResources().getColor(R.color.white));
        ...
        textDesPaint.setColor(getResources().getColor(R.color.white));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float temp = mCount / 100;
        String textstr = String.format("%.2f", temp);
        ;//mCount+"" ;//String.format("%.2f", mCount);

        Log.d(TAG, "onDraw: " + textstr);
...
//                * 然后调用postInvalidate()不停的绘制view。
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            if (interpolatedTime < 1.0f) {
                ...
                mCount = CommFunAndroid.toInt32(mText, 0);// Integer.parseInt(mText);
            }
            postInvalidate();
        }
    }
