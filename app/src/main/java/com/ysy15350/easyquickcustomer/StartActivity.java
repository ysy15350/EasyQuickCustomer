...
import api.model.AdImg;
import api.model.Version;
import base.BaseActivity;
import base.data.BaseData;
import base.data.BaseInfoHelper;
...
//        CommFunMessage.showWaitDialog("正在初始化...");
    ...
    public void adImgIndex() {
        publicApi.adImgIndex(1, new ApiCallBack() {
            @Override
            public void onSuccess(boolean isCache, Response response) {
                super.onSuccess(isCache, response);
                ...
                try {
                    if (response != null) {
                        ...
                        if (code == 200) {
                            //{"code":200,"result":[
                            // {"id":"2","name":"\u8da3\u6536\u6b3e\u5f00\u5c4f\u5e7f\u544a\u4e00","icon":"87","typeid":"2","sort":"11","url":"11","remark":"","status":"1","images":"\/Uploads\/Picture\/2017-10-31\/59f83b81074a2.jpg"}
                            // ],"message":"\u67e5\u8be2\u6210\u529f"}

                            List<AdImg> list = response.getData(new TypeToken<List<AdImg>>() {
                            }.getType());

                            if (list != null && !list.isEmpty()) {
                                AdImg adImg = list.get(0);
                                String img_url = ConfigHelper.getServerUrl(Config.isDebug, false) + adImg.getImages();
                                BaseData.setCache("adImg", img_url);
                                mHolder.setImageURL(R.id.img_ad, img_url, 720, 1005);
                            }
...
//                showMsg(msg);
    ...
    private void uploadErrorLog() {
        ...
        requestParams.addBodyParameter("appName", "easyquickcustom" + CommFunAndroid.getAppVersionName(getApplicationContext()));
